...
def main():
    ...
    while True:
        ...
        # gradcam heatmap
        if args.gradcam:
            gradcam = cam(transformed_tensor).squeeze() # type: ignore np.ndarray, (224, 224), dtype=np.uint8
            gradcam = cv2.resize(gradcam, (frame.shape[1], frame.shape[0])) # type: ignore
